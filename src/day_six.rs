...
sk";
    ...
    input
    .split("\n\n")
    .map(|forms| {let count : usize = forms.chars().filter(|c| c.is_alphabetic()).unique().count(); count})
    .collect();
    ...
    input
    .split("\n\n")
    .map(|forms| forms.chars().filter(|c| c.is_alphabetic()).unique().filter(|c| forms.matches(&c.to_string()).count() == forms.matches("\n").count()+1).count() )
    .collect();
    let count: usize = counts.iter().sum();
    println!("{}",count)
}